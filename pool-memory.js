...
export function recordPoolDeploy(poolAddress, deployData) {
  ...
  // Implementation of Senior Agent Rule: 3 consecutive losses = 24h blacklist
  const consecutiveLossCount = 3;
  if (entry.deploys.length >= consecutiveLossCount) {
    const recent3 = entry.deploys.slice(-consecutiveLossCount);
    const allLosses = recent3.every(d => d.pnl_pct != null && d.pnl_pct < 0);
    if (allLosses) {
      const reason = `${consecutiveLossCount} consecutive losses (Permanent Blacklist)`;
      entry.is_permanently_blacklisted = true;
      log("pool-memory", `PERMANENT BLACKLIST: ${entry.name} blocked forever due to 3 consecutive losses.`);
    }
  }
...
export function isPoolOnCooldown(poolAddress) {
  ...
  const db = load();
  const entry = db[poolAddress];
  if (entry?.is_permanently_blacklisted) return true;
