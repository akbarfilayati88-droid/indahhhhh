...
Returns the top N eligible pools ranked for evaluation.
Each pool includes:
- darwin_score (0-100) — learned ranking aid based on historically winning patterns
- darwin_drivers (top contributing signals) in signal_name=value (direction) format
Candidates have already passed hard disqualifiers.
...
Blacklisted tokens are filtered BEFORE the LLM even sees pool candidates.`,
  ...
  },
  {
    ...
    function: {
      name: "update_config",
      description: "Dynamically update a configuration attribute based on remote admin request. This edits `user-config.json` and memory instantly. Common valid keys: 'deployAmountSol', 'stopLossPct', 'takeProfitFeePct', 'screeningIntervalMin'.",
      parameters: {
        type: "object",
        properties: {
          key: { type: "string", description: "The exact camelCase key name in user config, e.g. 'deployAmountSol'" },
          value: { type: "number", description: "The new numeric value to set" }
        },
        required: ["key", "value"]
      }
    }
