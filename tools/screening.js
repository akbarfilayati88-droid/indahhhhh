...
export async function getTopCandidates({ limit = 10 } = {}) {
  ...
  // Darwin score (learned ranking aid) for the current shortlist.
  // Note: at this stage we don't yet have smart-wallet/narrative signals,
  // so those will be treated as neutral (0.5) by the scorer.
  if (eligible.length > 0) {
    const { computeDarwinRanking } = await import("../signal-weights.js");
    const darwin = computeDarwinRanking(eligible, {
      getSignals: (p) => ({
        organic_score: p.organic_score ?? null,
        fee_tvl_ratio: p.fee_active_tvl_ratio ?? p.fee_tvl_ratio ?? null,
        volume: p.volume_window ?? null,
        mcap: p.mcap ?? null,
        holder_count: p.holders ?? p.holder_count ?? null,
    ...
    for (let i = 0; i < eligible.length; i++) {
      eligible[i].darwin_score = darwin[i]?.darwin_score ?? null;
      eligible[i].darwin_drivers = darwin[i]?.darwin_drivers ?? [];
    }
  }
