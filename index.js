...
export async function runManagementCycle({ silent = false } = {}) {
  ...
  try {
    ...
    for (const p of positionData) {
      ...
      // Rule 1: stop loss (HARD RULE SECTION 2)
      if (!pnlSuspect && p.pnl_pct != null && p.pnl_pct <= -15) {
        actionMap.set(p.position, { action: "CLOSE", rule: 1, reason: "stop loss (-15%)" });
        continue;
      }
      // Rule 2: take profit (HARD RULE SECTION 2)
      if (!pnlSuspect && p.pnl_pct != null && p.pnl_pct >= 10) {
        actionMap.set(p.position, { action: "CLOSE", rule: 2, reason: "take profit (10%)" });
        continue;
      }
    ...
    if (actionPositions.length > 0) {
      ...
      const actionBlocks = actionPositions.map((p) => {
        const act = actionMap.get(p.position);
        const tracked = getTrackedPosition(p.position);
        const organic = p.organic_score ?? tracked?.organic_score ?? "unknown";
        const vol = p.volatility ?? tracked?.volatility ?? "unknown";
        return [
          ...
          `  metrics: organic=${organic} volatility=${vol} original_size=${tracked?.amount_sol ?? "unknown"} SOL`,
...
RULES (SECTION 5 — EXIT CHECKLIST):
- CLOSE (A, B, E, F): If these rules trigger, close immediately.
- REBALANCE (C): If OOR but metrics (organic >= 70, vol < 4.0) are good, REBALANCE.
- OOR CLOSE (D): If OOR and metrics are poor, CLOSE.
- Report following SECTION 6 TELEGRAM OUTPUT FORMAT.
- REBALANCE procedure: call close_position, then deploy_position at current bin with SAME tier size.
...
export async function runScreeningCycle({ silent = false } = {}) {
  ...
  try {
    ...
    log("cron", `Macro State fetched: ${macroState.split("\n")[0]}`);
    ...
    const passing = allCandidates.filter(({ pool, ti }) => {
      ...
      if (volatility != null && volatility >= volCap) {
        log("screening", `Volatility skip: dropped ${pool.name} — volatility ${volatility} >= ${volCap}`);
        filteredOut.push({ name: pool.name, reason: `volatility ${volatility} >= ${volCap} (HARD SKIP)` });
        return false;
      }

      // Implementation of Pinned Rule 5: Token must be within 10% of its 1-hour LOW
      if (pool.price != null && pool.min_price != null) {
        const priceAboveLowPct = ((pool.price - pool.min_price) / pool.min_price) * 100;
        if (priceAboveLowPct > 10) {
          log("screening", `Entry timing skip: dropped ${pool.name} — price is ${priceAboveLowPct.toFixed(2)}% above 1h LOW (limit 10%)`);
          filteredOut.push({ name: pool.name, reason: `Too far from 1h low (${priceAboveLowPct.toFixed(2)}% > 10%)` });
          return false;
        }
      }
    ...
    // Darwin ranking aid for the current shortlist (learned signal weights + drivers).
    // Use it only to prioritize evaluation order; hard deployment decisions still follow hard rules.
    try {
      const { computeDarwinRanking } = await import("./signal-weights.js");
      const darwin = computeDarwinRanking(passing, {
        getSignals: ({ pool, sw }) => ({
          organic_score: pool.organic_score ?? null,
          fee_tvl_ratio: pool.fee_active_tvl_ratio ?? pool.fee_tvl_ratio ?? null,
          volume: pool.volume_window ?? null,
          mcap: pool.mcap ?? null,
          holder_count: pool.holders ?? pool.holder_count ?? null,
      ...
      for (let i = 0; i < passing.length; i++) {
        passing[i].pool.darwin_score = darwin[i]?.darwin_score ?? null;
        passing[i].pool.darwin_drivers = darwin[i]?.darwin_drivers ?? [];
      }

      // Highest Darwin score first.
      passing.sort((a, b) => (b.pool.darwin_score ?? -1) - (a.pool.darwin_score ?? -1));
    } catch (e) {
      log("darwin", `Darwin ranking failed (continuing without it): ${e.message}`);
    }
    ...
    const candidateBlocks = passing.map(({ pool, sw, n, ti, mem }, i) => {
      ...
      const block = [
        ...
        `  darwin_score=${pool.darwin_score ?? "?"}/100 | darwin_drivers=${(pool.darwin_drivers ?? []).slice(0, 3).join(", ") || "n/a"}`,
    ...
    const { content } = await agentLoop(`
SCREENING CYCLE
ONLY Tier 3 (Score 75+) is authorized for automated deployment.
Sizing: Tier 3 = 0.3 SOL.
...
STEPS:
1. Apply SECTION 4 ENTRY CHECKLIST. Reject if any Required field fails. OR if Macro is BLOOD BATH.
2. ONLY Tier 3 (75+ Score) is allowed to deploy.
3. Call deploy_position with Tier 3 size (0.3 SOL).
   Evaluate Volatility to choose the DLMM shape (strategy) and bins_below dynamically.
4. Output ONLY the string format defined in SECTION 6 TELEGRAM OUTPUT FORMAT. Keep it highly aesthetic and elegant.
...
// -------- MACRO MARKET FETCHER -------- //
async function fetchMacroState() {
  try {
    const res = await fetch("https://api.dexscreener.com/latest/dex/tokens/So11111111111111111111111111111111111111112", {
      headers: { "User-Agent": "Mozilla/5.0" }
    });
    if (!res.ok) return "Unknown Macro State";
    const data = await res.json();
    const pair = data.pairs?.[0];
    if (!pair) return "Unknown Macro State";
    const h1 = pair.priceChange?.h1 ?? 0;
    const h24 = pair.priceChange?.h24 ?? 0;

    let condition = "STABLE 🟢";
    if (h1 <= -5 || h24 <= -10) condition = "BLOOD BATH 🔴 (EXTREME BEARISH) - STRICTLY RESTRICT ENTRY";
    else if (h1 <= -2 || h24 <= -5) condition = "CHOPPY 🟡 (BEARISH) - BE CAREFUL";
    else if (h1 >= 5 || h24 >= 10) condition = "BULLISH 🚀 - GOOD MOMENTUM";

    return `SOL Price: $${pair.priceUsd} | 1h: ${h1}% | 24h: ${h24}%\nMacro Condition: ${condition}`;
  } catch (e) {
    return "Unknown Macro State (Fetch failed)";
  }
}
...
function getTier(p) {
  ...
  // 1. Organic
  if (p.organic_score >= 90) score += 35;
  else if (p.organic_score >= 80) score += 25;
  else if (p.organic_score >= 70) score += 15;

  // 2. Fee/TVL
  const ftvl = (p.fee_active_tvl_ratio ?? p.fee_tvl_ratio ?? 0) * 100; // convert to percentage for comparison
  if (ftvl >= 30) score += 35;
  else if (ftvl >= 15) score += 25;
  else if (ftvl >= 8) score += 15;
  else if (ftvl >= 5) score += 5;

  // 3. Volatility
  if (p.volatility != null) {
    if (p.volatility < 2.0) score += 20;
    else if (p.volatility < 3.0) score += 10;
    else if (p.volatility < 4.0) score += 5;
  }

  // 4. Boosters
  if (p.token_age_hours != null && p.token_age_hours < 24) score += 5;
  // Note: Smart wallets present is hard to calculate synchronously here without the full pre-loaded object, 
  // so the terminal output might be missing up to 5 points compared to the AI, which is an acceptable safe margin.

  // 5. Penalties
  if (p.price != null && p.min_price != null) {
    const priceAboveLowPct = ((p.price - p.min_price) / p.min_price) * 100;
    if (priceAboveLowPct > 50) score -= 30;
    else if (priceAboveLowPct > 25) score -= 15;
  }
...
function formatCandidates(candidates) {
  ...
  const lines = candidates.map((p, i) => {
    const tier = getTier(p).padStart(2);
    const name = (p.name || "unknown").padEnd(17);
    const darwin = p.darwin_score == null ? "?" : String(p.darwin_score).padStart(3);
    const ftvl = `${p.fee_active_tvl_ratio ?? p.fee_tvl_ratio}%`.padStart(8);
    const vol = `$${((p.volume_window || 0) / 1000).toFixed(1)}k`.padStart(8);
    const active = `${p.active_pct}%`.padStart(6);
    const org = String(p.organic_score).padStart(4);
    return `  [${i + 1}] D${darwin}  T${tier}  ${name}  fee/aTVL:${ftvl}  vol:${vol}  in-range:${active}  organic:${org}`;
  });

  return [
    "  #   Darwin Tier pool              fee/aTVL     vol    in-range  organic",
    "  " + "─".repeat(72),
    ...lines,
  ].join("\n");
}
...
async function telegramHandler(msg) {
  ...
  const nyuruhMatch = text.match(/^\/nyuruh\s+(.+)$/is);
  if (!nyuruhMatch) return; // Only process general chatting/commands prefixed with /nyuruh
  const promptText = nyuruhMatch[1];

  busy = true;
  let liveMessage = null;
  try {
    const hasCloseIntent = /\bclose\b|\bsell\b|\bexit\b|\bwithdraw\b/i.test(promptText);
    const isDeployRequest = !hasCloseIntent && /\bdeploy\b|\bopen position\b|\blp into\b|\badd liquidity\b/i.test(promptText);
    const isSimpleChat = !hasCloseIntent && !isDeployRequest && /^(halo|hi|hey|halo|p|ping|test|apa kabar|p)$/i.test(promptText);
    const agentRole = isDeployRequest ? "SCREENER" : "GENERAL";
    const agentModel = agentRole === "SCREENER" ? config.llm.screeningModel : config.llm.generalModel;

    if (!isSimpleChat) {
      liveMessage = await createLiveMessage("🤖 Live Update", `Request: ${promptText.slice(0, 240)}`);
    }

    const { content } = await agentLoop(promptText, config.llm.maxSteps, sessionHistory, agentRole, agentModel, agentRole === "GENERAL" ? 2048 : null, {
      requireTool: !isSimpleChat, // Don't force tool use for simple chat
      interactive: true,
      onToolStart: async ({ name }) => { await liveMessage?.toolStart(name); },
      onToolFinish: async ({ name, result, success }) => { await liveMessage?.toolFinish(name, result, success); },
    });
    appendHistory(promptText, content);
