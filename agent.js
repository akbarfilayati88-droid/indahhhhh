...
const INTENT_TOOLS = {
  ...
  chat: new Set([]), // No tools needed for simple chat
};
...
const INTENT_PATTERNS = [
  ...
  { intent: "balance", re: /\b(balance|wallet|sol|how much|funds|money)\b/i },
  { intent: "positions", re: /\b(position|portfolio|open|pnl|yield|range|status|what.?s open)\b/i },
  { intent: "strategy", re: /\b(strategy|strategies)\b/i },
  { intent: "screen", re: /\b(screen|candidate|find pool|search|research|token|what.?s good|recommend)\b/i },
  ...
  { intent: "performance", re: /\b(performance|history|how.?s the bot|how.?s it doing|stats|report|summary)\b/i },
  { intent: "lessons", re: /\b(lesson|learned|teach|pin|unpin|clear lesson|what did you learn)\b/i },
  { intent: "chat", re: /\b(hello|hi|hey|how are you|what.?s up|status|alive|ping)\b/i },
];
...
export async function agentLoop(goal, maxSteps = config.llm.maxSteps, sessionHistory = [], agentType = "GENERAL", model = null, maxOutputTokens = null, options = {}) {
  ...
  // SKIP expensive lookups for simple chat/greetings
  const isPureChat = !TOOL_REQUIRED_INTENTS.test(goal) && agentType === "GENERAL";
  const [portfolio, positions] = isPureChat
    ? [null, null]
  ...
  for (let step = 0; step < maxSteps; step++) {
    ...
    try {
      ...
      const toolResults = await Promise.all(msg.tool_calls.map(async (toolCall) => {
        ...
        // Clean up keys with accidental spaces (e.g. "amount_ y" -> "amount_y")
        if (functionArgs && typeof functionArgs === "object" && !Array.isArray(functionArgs)) {
          for (const key of Object.keys(functionArgs)) {
            if (key.includes(" ")) {
              const fixedKey = key.replace(/\s+/g, "");
              if (functionArgs[fixedKey] === undefined) {
                functionArgs[fixedKey] = functionArgs[key];
              }
              delete functionArgs[key];
              log("warn", `Auto-fixed argument key spacing: "${key}" -> "${fixedKey}" for ${functionName}`);
            }
          }
        }
        ...
        // Only lock NO_RETRY_TOOLS if the execution was actually attempted without immediately throwing an error
        const wasBlocked = result?.blocked === true;
        const hadError = result?.error != null || result?.success === false;

        if (NO_RETRY_TOOLS.has(functionName) && !wasBlocked && !hadError) {
