...
/**
 * Compute Darwinian ranking score for a shortlist.
 *
 * Darwin score is a ranking aid for the *current shortlist*:
 * - Normalizes each numeric signal across the shortlist (min/max).
 * - Applies learned signal weights.
 * - volatility is treated as "lower is better".
 *
 * Returns per-candidate:
 * - darwin_score: integer 0-100
 * - darwin_drivers: string[] formatted `signal_name=value (direction)`
 */
export function computeDarwinRanking(candidates, { getSignals, topDrivers = 3 } = {}) {
  if (!Array.isArray(candidates)) return [];
  if (typeof getSignals !== "function") throw new Error("computeDarwinRanking requires getSignals(cand) -> signals");

  const data = loadWeights();
  const weights = data.weights || { ...DEFAULT_WEIGHTS };

  const signalsForCandidates = candidates.map((c) => getSignals(c) || {});

  // Build min/max normalization ranges per numeric signal.
  const NUMERIC_SIGNALS = new Set(["organic_score", "fee_tvl_ratio", "volume", "mcap", "holder_count", "volatility"]);
  const BOOLEAN_SIGNALS = new Set(["smart_wallets_present"]);

  /** @type {Record<string, { min: number|null, max: number|null }>} */
  const ranges = {};
  for (const s of SIGNAL_NAMES) {
    ranges[s] = { min: null, max: null };
  }

  for (const signal of SIGNAL_NAMES) {
    if (!NUMERIC_SIGNALS.has(signal) && !BOOLEAN_SIGNALS.has(signal)) continue;

    const vals = [];
    for (const snap of signalsForCandidates) {
      const v = snap?.[signal];
      if (BOOLEAN_SIGNALS.has(signal)) {
        if (v === true) vals.push(1);
        else if (v === false) vals.push(0);
        continue;
      }
      if (v != null && typeof v === "number" && isFinite(v)) vals.push(v);
    }
    ...
    if (vals.length >= 2) {
      ranges[signal].min = Math.min(...vals);
      ranges[signal].max = Math.max(...vals);
    }
  }
  ...
  function normNumeric(signal, raw) {
    if (raw == null || typeof raw !== "number" || !isFinite(raw)) return 0.5;
    const { min, max } = ranges[signal];
  ...
  function goodness(signal, normalized) {
    // Higher is better for most numeric signals, but volatility is lower-is-better.
    if (signal === "volatility") return 1 - normalized;
    return normalized;
  }

  function directionForDriver(signal, contribution) {
    if (signal === "volatility") {
      return contribution > 0 ? "positive" : "negative";
    }
    return contribution > 0 ? "bullish" : "bearish";
  }

  function formatDriverValue(signal, snapSignals, normalized) {
    const raw = snapSignals?.[signal];
    if (signal === "organic_score") {
      if (raw != null && typeof raw === "number" && isFinite(raw)) return clamp(raw / 100, 0, 1).toFixed(2);
      return normalized.toFixed(2);
    }
    if (signal === "volatility") {
      if (raw != null && typeof raw === "number" && isFinite(raw)) return raw.toFixed(2);
      return normalized.toFixed(2);
    }
    // Default: normalized 0-1 is consistent for drivers.
    return normalized.toFixed(2);
  }

  // Precompute denominator: sum of weights (signals not present are neutral via goodness=0.5).
  let denom = 0;
  for (const signal of SIGNAL_NAMES) denom += (weights?.[signal] ?? 1.0);
  denom = denom > 0 ? denom : 1;

  const results = [];
  for (let i = 0; i < candidates.length; i++) {
    const snapSignals = signalsForCandidates[i];

    const contributionBySignal = {};

    let weightedSum = 0;

    for (const signal of SIGNAL_NAMES) {
      const w = weights?.[signal] ?? 1.0;

      let normalized;
      if (NUMERIC_SIGNALS.has(signal)) normalized = normNumeric(signal, snapSignals?.[signal]);
      else if (BOOLEAN_SIGNALS.has(signal)) normalized = normBool(signal, snapSignals?.[signal]);
      else normalized = 0.5; // unknown/unmodeled signals treated as neutral

      const g = goodness(signal, normalized);

      // Contribution is how much this signal is pushing above/below neutral (0.5).
      const contribution = w * (g - 0.5);
      contributionBySignal[signal] = contribution;
      weightedSum += w * g;
    }

    const score01 = weightedSum / denom; // 0..1
    const darwin_score = Math.round(clamp(score01, 0, 1) * 100);

    const drivers = Object.entries(contributionBySignal)
      .filter(([, c]) => Math.abs(c) > 1e-9)
      .sort((a, b) => Math.abs(b[1]) - Math.abs(a[1]))
      .slice(0, topDrivers)
      .map(([signal, contribution]) => {
        // Recompute normalized for formatting.
        let normalized;
        if (signal === "volatility") normalized = normNumeric("volatility", snapSignals?.[signal]);
        else if (NUMERIC_SIGNALS.has(signal)) normalized = normNumeric(signal, snapSignals?.[signal]);
        else if (BOOLEAN_SIGNALS.has(signal)) normalized = normBool(signal, snapSignals?.[signal]);
        else normalized = 0.5;

        const val = formatDriverValue(signal, snapSignals, normalized);
        return `${signal}=${val} (${directionForDriver(signal, contribution)})`;
      });

    results.push({ darwin_score, darwin_drivers: drivers });
  }
