...
const _editCache = new Map(); // messageId -> lastText
...
async function postTelegram(method, body) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      // Silently ignore "message is not modified" error from editMessageText
...
export async function editMessage(text, messageId) {
  ...
  const newText = String(text).slice(0, 4096);
  if (_editCache.get(messageId) === newText) return null; // No change, skip API call
  _editCache.set(messageId, newText);
